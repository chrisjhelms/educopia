...
use CGI::Carp; # Debug mode: qw( fatalsToBrowser );
use XML::LibXML;
use XML::LibXSLT;
use WWW::Mechanize;


my $webIndex = 'uhttp://84.88.13.203:8080'; # = '.', si aquest script s'emplaça a la mateixa màquina que el repositori  
...
unless( $out->param ) {
	...
	my @communities = $mech->find_all_links( url_regex => qr/handle\/\d+\/\d+\/?$/i );
 	error($out,"No s'han trobat enllaços a comunitats a la web del repsoitori proporcionada") unless ( @communities ); 	
...
# L'script actua com a generador del fitxer csv que enumera tots els Handle IDs corresponents a Col.leccions de DSpace (=AUs=Dept. d'una Universitats), per a aquell HandleID d'una communitat concreta que se li passi comm2csv=hdl_id_comm. El csv resultant és el fitxer que s'haurà de pujar al conspectus per a indicar quines col·leccions de DSpace composen les AUs d'una col·lecció nostra a Metaarchive.
	...
	eval{
		$mech->follow_link ( url_regex => qr/handle\/\d+\/$commID\/?$/i );
                1;
        } or do {error($out,"Error de connexió/URL en seguir l'enllaç a la pàgina principal de la comunitat/institució $commID.")};	
	$mech->success or error ($out, "El repositori aha tornat un error en intentar accedir a la pàgina principal de la comunitat/institució $commID");
