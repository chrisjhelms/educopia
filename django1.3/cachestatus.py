...
class StatusScript(LockssScript):
    ...
    def __init__(self, argv0):
        LockssScript.__init__(self, argv0, '$Revision: $', StatusScript.MYCONFIGS) 
    ...
    def mkdir(self, action, server):
        ...
        if not os.path.exists(dirname):
            try: 
                ...
                log.info("using output directory %s" % dirname)           
        return dirname 
    ...
    def process_server(self, server):
        ...
        try: 
            ...
            if (Action.GETCOMMPEERS in options.action): pass
                #TODO: self.getcommpeers(options.dir, options.noquit)
            ...
            log.info("------ Stop Processing %s" % server) 
