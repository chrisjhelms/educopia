    ...
    def loadTry(ui, auId, datafresh): 
        ...
        try:
            ...
            if (reportDate - urlReport.reportDate < datafresh): 
                log.info("uptodate UrlReport available - not querying cache") 
                return urlReport
        except: 
            pass; 
    ...
    def printcsv(folder, auids, orderby, hdrs, sep, minrev = 1):
        ...
        for auid in auids:
            ...
            try: 
                ...
                if (urls.count() == 0):
                    log.warn("NOOP %s: file %s No Urls with version >= %s" % (Action.PRTURLLIST, f.name, minrev))
                ...
                try:
                    ...
                    log.info('Stop %s: file %s version %s' % (Action.PRTURLLIST, f.name, minrev))
                    f.close()
                except IndexError:
                    log.warn("NOOP %s: file %s No Urls at all" % (Action.PRTURLLIST, f.name))
...
class LockssCrawlStatus(models.Model):
    ...
    def __loadTry(ui, auId): 
        ...
        reportDate = datetime.now() 
        if (not status): 
            log.info("No CrawlStatus Info for %s %s" % (auId.cache, auId.auId) )
        else: 
            for s in status: 
                # work on status info 
                if (not s):
                ...
                try: 
                    ...
                    st['startTime'] = datetime.strptime(s['start'], UI_STRFTIME)
    ...
    def load(cache, auId, trials, sleep,timeout):
        success = False
        log.info('Start %s: %s ...' % (Action.GETCRAWLSTATUS, auId))
        try: 
            ...
            log.info('Stop %s: %s Success = %s ...' % (Action.GETCRAWLSTATUS, auId, success))
        return  success
    ...
    def printcsv(f, auids, sort, limit, hdrs, sep):
        ...
        log.info('Stop %s: File %s ...' % (Action.PRTCRAWLSTATUS, f.name))
