...
<html>
  ...
  <body>
    ...
    <p class="intro">You are viewing an HTML version of the XML OAI response. 
More information about this XSLT is at the <a href="#moreinfo">bottom of the page</a>.</p>
    ...
    <p>This stylesheet is used to convert OAI:DC XML produced by DSPACE instances into HTML pages that link to DSPACE item pages showing the full item record, ie to a URL with '?mode=full'. See more info at <a href="./manifestNYU.html"> manifestNYU.html </a>.
	</p> <p> 
...
<xsl:text> </xsl:text>
<a calass="link" href="{.}?verb=Identify">Identify</a></li>
</xsl:template>
...
<tr><td class="key">Resource Identifier</td><td class="value">
  <xsl:choose>
    <xsl:when test='starts-with(.,"http" )'>
       <xsl:variable name='dspaceitem' select="." />
       <a class="link" href="{$dspaceitem}?mode=full"> 
       <xsl:value-of select="."/>
       </a> 
    </xsl:when>
    <xsl:otherwise>
